...
function DraggableLI({ id, stat, index, moveLi, stats}) {
            ...
            for(z = 0; z < stats.length; z++) {
                ...
                if(statUpdate != null){
                    ...
                    if(statStr.match(/-?\d+/g) != null) {
                        ...
                        let newStat = parseInt(document.getElementById(stats[z]).innerHTML, 10)   
                        statUpdate.innerHTML = "&nbsp;"+(newStat + baseStat[2]) + "&nbsp;&nbsp;(" + newStat + " + " + baseStat[2] + ")" 
                    }
                }
            }
        }
	...
	return (
	    <>
	    	<li id={stat} ref={ref} style={{ ...style, opacity }} className="character-sheet-stat text-center text-glow"></li>
	    </>
    )
}
