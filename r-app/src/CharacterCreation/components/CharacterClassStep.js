...
function CharacterClass({races}) {
	...
	function setModalIsOpen(key){
		setModalState(prevState => ({
            ...prevState,0: false,1: false,2: false,3: false,4: false,5: false,6: false,7: false,8: false,9: false,10: false,11: false,12: false,
        }));
		setModalState(prevState => ({
            ...prevState,
            [key]: true
	...
	function rollBaseStats(key) {
		...
		var brawnMod = 0, agilityMod = 0, vitalityMod = 0, egoMod = 0, moxieMod = 0, luckMod = 0, wildcards = 0
		...
		for(let i = 0; i < backendData.attributes.length; i++) {
			switch(backendData.attributes[i][1]) {
				...
				case "wildcard":
					...
					break;
				default:
					console.log("Failed to match case!")
			}
		}

		document.getElementById('brawn-0').innerHTML = "&nbsp;"+(brawn + brawnMod) + "&nbsp;&nbsp;(" + brawn + " + " + brawnMod + ")"
		document.getElementById('agility-1').innerHTML = "&nbsp;"+(agility + agilityMod) + "&nbsp;&nbsp;(" + agility + " + " + agilityMod + ")"
		document.getElementById('vitality-2').innerHTML = "&nbsp;"+(vitality + vitalityMod) + "&nbsp;&nbsp;(" + vitality + " + " + vitalityMod + ")"
		document.getElementById('ego-3').innerHTML = "&nbsp;"+(ego + egoMod) + "&nbsp;&nbsp;(" + ego + " + " + egoMod + ")"
		document.getElementById('moxie-4').innerHTML = "&nbsp;"+(moxie + moxieMod) + "&nbsp;&nbsp;(" + moxie + " + " + moxieMod + ")"
		document.getElementById('luck-5').innerHTML = "&nbsp;"+(luck + luckMod) + "&nbsp;&nbsp;(" + luck + " + " + luckMod + ")"
		...
		document.getElementById('physical-defense').innerHTML = (backendData.subAttributes[0] ? backendData.subAttributes[0][0] : "")
				    ...
				    <ul className="basic-list">
					    {races[1].map((race, key) => (
					    	<li key={key} onClick={() => setModalIsOpen(key)}>
					    		...
					    		<Modal id={"modal-"+key} isOpen={modalIsOpen[key]} style={customStyles}>
						    		<div className="container">
							    		<div className="row">
							    			<div className="col-sm-6 text-glow">
							    				...
							    				{race[1].attributes}<br/>

							    				<button onClick={() => rollBaseStats(key)}>Generate Stats</button>
							    			</div>
